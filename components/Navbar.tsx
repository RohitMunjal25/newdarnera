...
export default function Navbar() {
  ...
  return (
    <>
      <nav 
        className={`sticky top-0 left-0 w-full z-[99999] transition-all duration-500 border-b ${
          isScrolled 
            ? 'bg-black/95 backdrop-blur-md border-gray-800 shadow-2xl py-4' 
      ...
      {/* Mobile Slide-over Drawer (Rendered at root level with extreme high z-index to overlay everything perfectly) */}
      {isMobileMenuOpen && (
          ...
          <div className="relative w-[85%] max-w-sm h-full bg-[#080808] border-l border-gray-800 p-8 flex flex-col justify-between shadow-2xl z-10 overflow-y-auto">
            ...
            <div>
              <div className="flex justify-between items-center mb-10 pb-4 border-b border-gray-900">
                ...
                <button 
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="w-9 h-9 rounded-full bg-black border border-gray-800 text-gray-400 hover:text-white flex items-center justify-center text-sm cursor-pointer shadow-md"
                >
                  ✕
                </button>
              </div>
